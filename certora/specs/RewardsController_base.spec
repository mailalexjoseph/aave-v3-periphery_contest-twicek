...
///////////////// Properties ///////////////////////
    ...
    // //Update UserData mapping should imply accrued rewards
    // rule userIndex_accrued() {
    //     env e;
    ...
    //     //Get rewardData
    //     indexBefore, emissionPerSecondBefore, lastUpdateTimestampBefore, distributionEndBefore = getRewardsData(e, AToken, reward);
    //     uint128 userAccruedBefore = getRewardAccruedForUser(e, AToken, reward, e.msg.sender);
    ...
    //     uint128 userAccruedAfter = getRewardAccruedForUser(e, AToken, reward, e.msg.sender);


    //     require AToken.decimals(e) != 0;
    //     mathint expression = userBalance * (indexAfter - userIndex) / (10^AToken.decimals(e));
    //     require assert_uint128(expression) < max_uint128;

    //     assert userAccruedAfter == assert_uint128(expression);
    // }

    // A AToken balance is needed to update rewards?
    // rule ATokenBalance_needed_for_rewards() {
    //     env e;
    ...
    //     uint128 accrued = getRewardAccruedForUser(e, AToken, reward, e.msg.sender);
