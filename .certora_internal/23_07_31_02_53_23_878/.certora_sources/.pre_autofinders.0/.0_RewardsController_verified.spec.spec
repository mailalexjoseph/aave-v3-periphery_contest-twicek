...
///////////////// Properties ///////////////////////
    ...
    rule accruedRewardsSent() {
        ...
        uint256 balanceBefore = to_uint256(reward.balanceOf(e, user));

        rewardsList, claimedAmounts = claimAllRewards(e, assets, user);

        uint256 balanceAfter = to_uint256(reward.balanceOf(e, user));

        require balanceBefore <= balanceAfter;

        uint256 balance = balanceBefore - balanceAfter;
        //Is accrued reward == claimedAmounts
        assert claimedAmounts[0] == balance;
    }
