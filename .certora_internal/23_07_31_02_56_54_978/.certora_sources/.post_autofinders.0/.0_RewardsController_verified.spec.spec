...
///////////////// Properties ///////////////////////
    ...
    rule accruedRewardsSent() {
        ...
        mathint balanceBefore = reward.balanceOf(e, user);

        rewardsList, claimedAmounts = claimAllRewards(e, assets, user);

        mathint balanceAfter = to_uint256(reward.balanceOf(e, user));

        require balanceBefore <= balanceAfter;

        mathint balance = balanceBefore - balanceAfter;
        uint256 test = to_uint256(balance);
        //Is accrued reward == claimedAmounts
        assert claimedAmounts[0] == test;
    }
